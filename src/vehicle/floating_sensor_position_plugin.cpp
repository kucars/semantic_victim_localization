...
void FloatingSensorPosition::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  pub_pose_stamped = this->rosNode->advertise<geometry_msgs::PoseStamped>(this->topic_base_ + "/poseStamped", 10);
...
/// of the Velodyne.
void FloatingSensorPosition::OnRosMsgPose(const geometry_msgs::PoseStampedConstPtr &msg)
{
  // Lock critical section
  mtx_pose_.lock();
  ...
  // Update positions
  current_pos_x_ = msg->pose.position.x;
  current_pos_y_ = msg->pose.position.y;
  current_pos_z_ = msg->pose.position.z;

  // Convert quaternion to Euler angles (w,x,y,z notation)
  math::Quaternion q = math::Quaternion(msg->pose.orientation.w, msg->pose.orientation.x, msg->pose.orientation.y, msg->pose.orientation.z);
  math::Vector3 rpy = q.GetAsEuler();
...
//    GetCurrentPose();
  ...
  if (pub_pose_stamped.getNumSubscribers() > 0)
  {
    current_pose_.position.x = current_pos_x_;
    current_pose_.position.y = current_pos_y_;
    current_pose_.position.z = current_pos_z_;
...
//    GetCurrentPose();
    ...
    pub_pose_stamped.publish(current_pose_Stamped);
  }
}
