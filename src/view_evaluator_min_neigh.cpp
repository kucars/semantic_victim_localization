...
view_evaluator_MinNEIGH::view_evaluator_MinNEIGH():
  view_evaluator_base()
{
...
double view_evaluator_MinNEIGH::calculateUtility(geometry_msgs::Pose p, Victim_Map_Base *mapping_module){
  ...
  for (grid_map::GridMapIterator iterator(mapping_module_->map); !iterator.isPastEnd(); ++iterator) {
    ...
    if(temp_Map.atPosition("temp", position)==0){
      ...
      if(min_belief>curr_pro)
       min_belief=curr_pro;

      if(max_belief<curr_pro)
       max_belief=curr_pro;
  }
}
  if (!std::isinf(min_belief))
    Info_view=max_belief-min_belief;

  return Info_view;
}
void view_evaluator_MinNEIGH::evaluate(){
...
std::string view_evaluator_MinNEIGH::getMethodName()
{
  return "MINNEIGH";
}
